## Essential data to find the magic nonce

version + prev_block + merkle_root + timestamp + size_bits

In that command, we skip the first 8 bytes and print the next 76 bytes to exclude the nouce found.
```
